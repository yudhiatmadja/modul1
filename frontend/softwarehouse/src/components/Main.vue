<template>
      ...
      <div class="flex flex-col md:flex-row items-center justify-between">
        <div class="flex flex-col gap-y-10">
          <div class="w-fit mt-14 px-3 py-2 small-badge flex flex-row bg-white rounded-full">
            <img src="../assets/images/world.png" alt="" class="w-6" />
          ...
          <div class="flex flex-row gap-x-6 items-center">
           <a href="/order" class="bg-violet-800 hover:bg-indigo-950 text-lg text-white py-4 px-10 rounded-full font-semibold">Build Now</a>
            <!-- <button id="build" class="bg-violet-800 hover:bg-indigo-950 text-lg text-white py-4 px-10 rounded-full font-semibold" @click="showModal">Build Now</button> -->
            <dialog v-if="isModalVisible" class="modal">
              <div class="modal-box">
                ...
                <form @submit.prevent="submitForm">
                  <div class="mb-5">
                    ...
                    <input type="text" v-model="formData.name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3" placeholder="Input your name" required />
                  </div>
                  <div class="mb-5">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-700">Your Email</label>
                    <input type="email" v-model="formData.email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3" placeholder="name@flowbite.com" required />
                  </div>
                  <div class="mb-5">
                    <label for="number" class="block mb-2 text-sm font-medium text-gray-700">Phone Number</label>
                    <input type="number" v-model="formData.number" id="number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3" placeholder="08xxxxxxx" required />
                  </div>
                  <div class="mb-5">
                    <label for="description" class="block mb-2 text-sm font-medium text-gray-700">Deskripsi</label>
                    <textarea v-model="formData.description" id="description" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3" required></textarea>
                  </div>
                  <div class="mb-5">
                    <label for="domain" class="block mb-2 text-sm font-medium text-gray-700">Choose your domain</label>
                    <select v-model="formData.domain" id="domain" class="bg-gray-50 mb-5 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                      <option selected disabled>Choose your domain</option>
                      <option v-for="domain in domains" :key="domain.domain_name" :value="domain.domain_name">{{ domain.domain_name }}</option>
                    </select>
                  </div>
  
                  <div class="mb-5">
                    <label for="payment_proof" class="block mb-2 text-sm font-medium text-gray-700">Upload Bukti Pembayaran</label>
                    <input type="file" id="payment_proof" @change="handleFileUpload" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    <p class="mt-1 text-sm text-gray-500">Format yang diperbolehkan: JPG, PNG, PDF (maks 5MB).</p>
                  </div>
  
                  <div class="flex flex-row items-center gap-x-3">
                    <button type="submit" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5">Order</button>
                    <button type="button" @click="cancelOrder" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5">Cancel</button>
                  </div>
                </form>
              </div>
            </dialog>
            <a href="#domain" class="flex gap-x-2">
              <img src="../assets/images/demo.png" alt="" class="w-6" />
              <p class="text-violet-700 font-bold">Pilih Domain</p>
            </a>
          </div>
        </div>
        <div class="flex flex-row items-center">
          <img src="../assets/images/hero_img.png" alt="" class="w-[550px]" />
        </div>
      </div>
    </section>
    ...
    <section class="partners max-w-6xl mx-auto py-12" id="partner">
      ...
      <div class="flex flex-col md:flex-row justify-between items-center gap-y-2">
        <img src="../assets/images/adclipse.png" alt="" class="w-32" />
        <img src="../assets/images/gopayy.png" alt="" class="w-32" />
        <img src="../assets/images/bca.png" alt="" class="w-20" />
        <img src="../assets/images/ovo.png" alt="" class="w-20" />
        <img src="../assets/images/visa.png" alt="" class="w-20" />
      </div>
    </section>
    ...
    <section class="project max-w-6xl mx-auto py-12" id="proyek">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 my-12">
        <div class="rounded-2xl relative group overflow-hidden">
          <img src="../assets/images/project/game.png" alt="" class="w-full h-auto" />
        ...
        <div class="flex flex-col rounded-2xl relative overflow-hidden group">
          <img src="../assets/images/project/blog.png" alt="" class="w-full h-auto" />
          <div class="flex justify-center w-full absolute inset-x-0 bottom-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out">
            <a href="https://github.com/yudhiatmadja" target="_blank" class="bg-violet-700 text-white px-7 py-3 rounded-full shadow-2xl shadow-violet-700 hover:bg-violet-800 transition duration-300 ease-in-out">View Details</a>
          </div>
        </div>
        ...
        <div class="flex flex-col rounded-2xl relative overflow-hidden group">
          <img src="../assets/images/project/web3.png" alt="" class="w-full h-auto" />
          <div class="flex justify-center w-full absolute inset-x-0 bottom-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out">
            <a href="https://github.com/yudhiatmadja" target="_blank" class="bg-violet-700 text-white px-7 py-3 rounded-full shadow-2xl shadow-violet-700 hover:bg-violet-800 transition duration-300 ease-in-out">View Details</a>
          </div>
        </div>
    ...
    <section class="domain max-w-6xl mx-auto py-12" id="domain">
      ...
      <div class="grid grid-rows-1 gap-y-2 md:grid-cols-3 gap-x-8">
        <div v-for="domain in domains" :key="domain.domain_name" class="carddomain flex flex-col gap-y-5 bg-white rounded-2xl p-5 hover:scale-110 duration-200">
      ...
      <div class="grid grid-cols-1 gap-y-5 md:grid-cols-2 gap-x-10">
        <div class="flex flex-col gap-y-10">
          ...
          <div class="flex flex-row gap-x-6 items-center">
            ...
            <a href="" class="flex gap-x-2">
              <img src="../assets/images/demo-white.png" alt="" class="w-6" />
        ...
        <div class="grid grid-cols-3 gap-x-5 gap-y-5">
          <div class="group hover:bg-[#080c2e] cardfitur bg-white p-5 rounded-2xl flex items-center flex-col gap-y-5 transition duration-300 ease-in-out">
            <img src="../assets/images/fitur/crawl.png" alt="" />
          ...
          <div class="group hover:bg-[#080c2e] cardfitur bg-white p-5 rounded-2xl flex items-center flex-col gap-y-5 transition duration-300 ease-in-out">
            <img src="../assets/images/fitur/report.png" alt="" />
            <div class="flex flex-col items-center">
          ...
          <div class="group hover:bg-[#080c2e] cardfitur bg-white p-5 rounded-2xl flex items-center flex-col gap-y-5 transition duration-300 ease-in-out">
            <img src="../assets/images/fitur/ddos.png" alt="" />
            <div class="flex flex-col items-center">
          ...
          <div class="group hover:bg-[#080c2e] cardfitur bg-white p-5 rounded-2xl flex items-center flex-col gap-y-5 transition duration-300 ease-in-out">
            <img src="../assets/images/fitur/scale.png" alt="" class="w-10" />
            <div class="flex flex-col items-center">
              <h3 class="font-bold text-center group-hover:text-white transition duration-300 ease-in-out">Auto-Scale</h3>
              <a href="" class="text-base font-semibold text-gray-500 group ```vue
  -hover:block hidden group-hover:text-white transition duration-300 ease-in-out">Learn More</a>
            </div>
          </div>
          <div class="group hover:bg-[#080c2e] cardfitur bg-white p-5 rounded-2xl flex items-center flex-col gap-y-5 transition duration-300 ease-in-out">
            <img src="../assets/images/fitur/ai.png" alt="" />
            <div class="flex flex-col items-center">
          ...
          <div class="group hover:bg-[#080c2e] cardfitur bg-white p-5 rounded-2xl flex items-center flex-col gap-y-5 transition duration-300 ease-in-out">
            <img src="../assets/images/fitur/plugin.png" alt="" />
            <div class="flex flex-col items-center">
  ...
  export default {
    data() {
      return {
        ...
        domains: [], // Populate this with your domain data
      };
    },
    methods: {
      showModal() {
      ...
      submitForm() {
        ...
        this.isModalVisible = false; // Close modal after submission
      },
      handleFileUpload(event) {
        this.formData.payment_proof = event.target.files[0];
      },
      cancelOrder() {
        this.isModalVisible = false; // Close modal on cancel
      },
    },
  };
  </script>
